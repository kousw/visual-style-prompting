...
from modules.models.attention_processor import ReferenceSelfAttnProcessor
...
class StableDiffusionXLPipeline(
    ...
    def __init__(
        ...
        reference_start_layer_index: int = 23,
    ):
        super().__init__()
        ...
        self.reference_start_layer_index = reference_start_layer_index
        self.load_attention_module(reference_start_layer_index)
    ...
    def __call__(
        ...
        reference_prompt: Union[str, List[str]] = None,
        ...
        **kwargs,
        ...
        if batch_size > 1:
            raise ValueError("Batch size > 1 is not supported for now.")
        ...
        if reference_images is not None:
            reference_images_for_unet_ref = self.reference_image_processor.preprocess(
                reference_images
            )
            ...
            self.processor.set_reference_index(batch_size)
        else:
        ...
        if self.do_classifier_free_guidance:
            if reference_images is not None:
                ...
                add_text_embeds = torch.cat(
                    [negative_pooled_prompt_embeds, add_text_embeds, add_text_embeds],
                    dim=0,
                )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # 2回目以降は、reference_latentsが含まれているが、再度ノイズをかけて追加するため、除外する
                # if reference_latents is not None and latents.shape[0] == batch_size + 2 if self.do_classifier_free_guidance else batch_size + 1:
                #     latents = latents[:-2]

                # expand the latents if we are doing classifier free guidance
                if self.do_classifier_free_guidance:
                    latent_model_input = torch.cat(
                        [latents, latents, reference_latents]
                    )  # uncond, cond, reference
                else:
                    latent_model_input = torch.cat([latents, reference_latents])
                ...
                # perform guidance
                if self.do_classifier_free_guidance:
                    noise_pred_uncond, noise_pred_text, noise_pred_ref = (
                        noise_pred.chunk(3)
                    )
    ...
    def load_attention_module(self, start_layer_index):
        self.processor = ReferenceSelfAttnProcessor()

        att1_layer_index = 0
        for name, module in self.unet.up_blocks.named_modules():
            if hasattr(module, "attn1"):
                if att1_layer_index >= start_layer_index:
                    print("Loading attention module for layer", name, att1_layer_index)
                    module.attn1.set_processor(ReferenceSelfAttnProcessor())
